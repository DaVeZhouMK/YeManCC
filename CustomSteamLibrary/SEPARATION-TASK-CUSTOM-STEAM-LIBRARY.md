...
> 当前切换点（2026-08-25）：运行时接入和 green-child 自动更新均已进入源码启用、分步验收。CustomSteamLibrary 独立包默认位于 `C:\SOFT\YeMan\YeManCC\CustomSteamLibrary`，数据规则不改变；主程序现有固定桥接路径和升级器本轮不修改。升级完成必须同时通过 Custom 子程序健康握手和 YeManCC 主程序握手；正式目录升级另行执行。
