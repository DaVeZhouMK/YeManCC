...
- 支持安全的通配符模式。
- `#` 后内容视为注释，空行忽略。
- 用户黑名单是默认排除条件；界面保存用户黑名单或白名单时会自动删除另一侧的冲突规则，避免同一进程同时存在两种用户判定。
- 黑名单命中后，进程不能成为阀门目标。
...
- 白名单与用户黑名单在正常界面保存路径下保持同名互斥；通配符规则只在识别时按各自名单匹配，若外部直接编辑造成匹配重叠，运行时仍按白名单优先处理。
- 白名单候选最低工作集为 `50 MB`。
- 普通候选最低工作集为 `500 MB`。
- 存在有效白名单候选时，白名单候选优先于普通工作集候选。
- 同一类别内仍按工作集较大者竞争。
- 白名单可以覆盖内置/系统黑名单，但仍需满足白名单候选的最低工作集门槛；用户黑名单冲突由保存入口自动清理。
