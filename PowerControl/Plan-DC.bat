...
REM ============================================================
REM  YeMan Plan-DC 脚本
REM  直接读取 tdp-dc.txt 设置 TDP（AMD + Intel），然后切换 ELITE 电源
REM  不做 AC/DC 判断，调用即按 DC 配置执行
REM  TDP 配置（单位：瓦 W，可加 # 注释行）：
REM    C:\SOFT\YeMan\PowerControl\tdp-dc.txt
...
REM ----- 1. 读取 tdp-dc.txt 并换算 AMD + Intel -----
call :SET_TDP
...
:SET_TDP
    ...
    echo [TDP-DC] 源=%SRC% 目标=%W%W (经 PawnIO, 已避开 WinRing0)
