...
function Read-HwinfoAll {
    ...
    try {
        ...
        # 读到任一核心信号即视为共享内存 OK（TDP/频率/温度任一有效）
        if ($res.tdpW -gt 0 -or $res.freqMhz -gt 0 -or $res.tempC -gt 0) { $res.ok = $true }
        else { $res.err = 'no CPU sensors found' }
    } finally {
        if ($acc) { $acc.Dispose() }
        $mmf.Dispose()
    }
...
while ($true) {
    ...
    try {
        ...
        if (-not $hw.ok -and -not $hwPrereqMissing) {
            if (-not (Test-Path -LiteralPath $HWINFO_EXE -PathType Leaf) -or -not (Test-Path -LiteralPath $HWINFO_CFG -PathType Container)) {
                $hwPrereqMissing = $true
                $hwDown = $true
            } elseif (-not $recoveryAttempted -or ([DateTimeOffset]::Now.ToUnixTimeMilliseconds() - $recoveryTs) -ge 3000) {
                $recoveryAttempted = $true
                $recoveryTs = [DateTimeOffset]::Now.ToUnixTimeMilliseconds()
                # 不再固定等待；立即按当前进程状态选择事务模式。
                $hwRunning = $false
                try { $hwRunning = @(Get-Process HWiNFO64 -ErrorAction SilentlyContinue).Count -gt 0 } catch { }
                $mode = if ($hwRunning) { 'restart' } else { 'start' }
                try { & cmd.exe /c "$HWINFO_BAT $mode" 2>$null } catch { }
                # 快速轮询 6 秒确认共享内存恢复；全局互斥保证不会与 autofloat 交叉杀拉。
                for ($i = 0; $i -lt 60; $i++) {
                    $hw = Read-HwinfoAll
