...
import { readTopMonitor, type TopMonData } from '@/bridge/topmon';
...
const showMonitor = ref(localStorage.getItem('yeman.ui.schedule-monitor') !== 'false');
watch(showMonitor, (v) => {
  localStorage.setItem('yeman.ui.schedule-monitor', String(v));
});
const topMon = ref<TopMonData | null>(null);
...
// sub 根据编辑面板当前 tdpMax 实时计算「执行瓦数」。
const TDP_STRATEGY_ORDER: TdpFloatStrategy[] = ['none', 'small', 'medium', 'large', 'aggressive'];
const TDP_STRATEGY_OPTS = computed(() => {
  const maxW = editingDraft.value?.tdpMax ?? 200;
  return TDP_STRATEGY_ORDER.map((s) => {
    const p = TDP_FLOAT_STRATEGIES[s];
    let effectiveW: number;
    if (p.ratio === 0) {
      effectiveW = maxW;
    } else {
      const drop = Math.max(p.minDrop, Math.ceil(maxW * p.ratio));
      effectiveW = Math.max(5, Math.floor(maxW - drop));
    }
    return { value: s, label: STRATEGY_LABEL[s], sub: `执行 ${effectiveW}W` };
  });
});
...
function modeDetail(side: PowerSide, mode: ScheduleMode): string {
  ...
  return `${fps} · ${p.tdpMax}W · ${cpuControlLabel(p)} · ${STRATEGY_LABEL[p.tdpStrategy]}`;
}
...
async function refreshMouseState() {
  const on = await isJoyxoffRunning();
  mouseOn.value = on;
  window.dispatchEvent(new CustomEvent('gp:mouse-mode', { detail: { on } }));
}
  ...
  try {
    for (;;) {
      ...
      try {
        ...
        if (shouldApply) await applyPerformanceSchedule(side, mode, snapshot);
...
async function refreshStatus() {
  const [tm, params] = await Promise.all([
    readTopMonitor().catch(() => null),
    readPowerParams().catch(() => null),
  ]);
  topMon.value = tm;
...
async function toggleMouse() {
  ...
  try {
    mouseOn.value = await toggleJoyxoff();
    window.dispatchEvent(new CustomEvent('gp:mouse-mode', { detail: { on: mouseOn.value } }));
    ...
    <Teleport to="body">
      <Transition name="rc-pop">
        ...
        >
          ...
          <p class="rc-desc">将重制 AC / DC 下节能、平衡、高性能、极致性能的全部 CPU、TDP、帧率和浮动设置。此操作不可撤销，是否继续？</p>
