...
let yemanGuardBusy = false;
let refreshInProgress = false;
let stopSchemeWatch: (() => void) | null = null;
let wakeSchemeTimer: number | null = null;
const WAKE_SCHEME_WINDOW_MS = 60_000;
const WAKE_SCHEME_INTERVAL_MS = 5_000;
async function enforceYemanScheme() {
  if (!isYemanScheme.value || yemanGuardBusy || refreshInProgress) return;
  yemanGuardBusy = true;
  try {
    const current = (await getActiveScheme()).toLowerCase();
    if (current && current !== PW.YEMAN.toLowerCase()) {
      await switchScheme('yeman');
      if (globalRefreshKey) globalRefreshKey.value++;
      await refresh();
    }
  } catch {
    // 检测或恢复失败不进入重试死循环，下一轮继续只读检测。
  } finally {
    yemanGuardBusy = false;
  }
}
function clearWakeSchemeChecks(): void {
  if (wakeSchemeTimer !== null) window.clearInterval(wakeSchemeTimer);
  wakeSchemeTimer = null;
}
function startWakeSchemeChecks(): void {
  clearWakeSchemeChecks();
  // 唤醒后的奥创可能延迟切换方案：60 秒内每 5 秒检查一次，共 12 次。
  let remaining = Math.ceil(WAKE_SCHEME_WINDOW_MS / WAKE_SCHEME_INTERVAL_MS);
  wakeSchemeTimer = window.setInterval(() => {
    if (remaining-- <= 0) {
      clearWakeSchemeChecks();
      return;
    }
    void enforceYemanScheme();
  }, WAKE_SCHEME_INTERVAL_MS);
}
function onPowerSchemeChanged(): void {
  void enforceYemanScheme();
}
...
const stopSchemeWatchEvent = on('power.schemeChanged', onPowerSchemeChanged);
const stopResumeSchemeWatch = on('power.resumed', startWakeSchemeChecks);
...
async function onCpuProfile(id: CpuProfileId): Promise<void> {
  if (!isYemanScheme.value || !paramsOk.value || busy.value || floatOn.value) return;
...
const offFloatUpdate = onFloatUpdate((info) => {
  ...
  if (info.enabled !== wasOn) {
    if (info.enabled) {
      void setActiveScheme(PW.YEMAN)
        .catch(() => {})
        .finally(() => refreshScheme());
    } else {
      void refreshScheme();
    }
  }
...
onUnmounted(stopSchemeWatchEvent);
onUnmounted(stopResumeSchemeWatch);
onUnmounted(clearWakeSchemeChecks);
...
<template>
  <div class="page">
    ...
    <section class="card profile-card" :class="{ disabled: !isYemanScheme }">
      <div class="profile-head">
        ...
        <button
          ...
          :disabled="!isYemanScheme || busy || floatOn"
      ...
      <div class="profile-buttons">
        <button
          ...
          :disabled="!isYemanScheme || !paramsOk || busy || floatOn"
    ...
    <section class="card" :class="{ disabled: !isYemanScheme }">
      ...
      <Slider v-model="acFreq" :min="0" :max="acMax" :step="100" :label="''" :unit="'MHz'" color="ac" :disabled="!isYemanScheme || !paramsOk || floatOn" @commit="onFreqCommit('ac')" />
      <Slider v-model="acAggr" :min="0" :max="100" :step="1" :label="'CPU主频调度积极性 (越大CPU主频越高)'" icon="plug" color="ac" :disabled="!isYemanScheme || !paramsOk || floatOn" @commit="onAggrCommit('ac')" />
    ...
    <section class="card" :class="{ disabled: !isYemanScheme }">
      ...
      <Slider v-model="dcFreq" :min="0" :max="dcMax" :step="100" :label="''" :unit="'MHz'" color="dc" :disabled="!isYemanScheme || !paramsOk || floatOn" @commit="onFreqCommit('dc')" />
      <Slider v-model="dcAggr" :min="0" :max="100" :step="1" :label="'CPU主频调度积极性 (越大CPU主频越高)'" icon="battery" color="dc" :disabled="!isYemanScheme || !paramsOk || floatOn" @commit="onAggrCommit('dc')" />
      <div class="profile-editing">正在编辑：{{ activeCpuProfileLabel }}挡位 · DC</div>
    </section>

    <section class="card core-mode-card" :class="{ disabled: !isYemanScheme }">
