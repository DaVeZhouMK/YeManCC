...
const BACKGROUND_OPACITY_KEY = 'yeman.ui.background-opacity';
const BACKGROUND_BLUR_KEY = 'yeman.ui.background-blur';
...
export function getBackgroundOpacity(): number {
  ...
  if (raw === null) return BACKGROUND_OPACITY_DEFAULT;
  return normalizeBackgroundOpacity(Number(raw));
}
...
export function getBackgroundBlur(): number {
  ...
  if (raw === null) return BACKGROUND_BLUR_DEFAULT;
  return normalizeBackgroundBlur(Number(raw));
}
