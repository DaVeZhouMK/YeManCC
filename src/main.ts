...
// WebView2 launch. Tell the native host after the Vue shell has mounted and two
// browser paint opportunities are ready. Do not wait for router.isReady(): the
// first route is lazy-loaded and can otherwise hold the native first-frame gate
// while the shell is already drawable. Background initialization remains async.
async function signalInitialRenderReady(): Promise<void> {
  ...
  try {
    ...
    await invoke(
      'window.renderReady',
      { generation: context.generation, navigationId: context.navigationId },
      { timeoutMs: 1500 },
    );
  } catch (error) {
